...
[TanStack Query](https://tanstack.com/query/latest), also known as **React Query** is described as the missing data-fetching library for web applications. **React Query** makes fetching, caching, and synching, and updating server state a breeze.

React Query comes with an opinionated way for fetching and updating data. React Query makes it easier to manage complex data fetching and cachinig scenarios in our React applications, while providing a simple and intuitive API as compared to traditional state management libraries. Using React Query also has other benefits such as:
...
- **Type safety** - React Query is built with TypeScript and provides strong type safety. This makes it easier to catcherrors and avoid bugs.
...
- **Automatic data refetching** - React Query can automatically refetch data from the server based several conditions, such as when a mutation is performed, the window is refocused, or a component is mounted.
...
```
and follow the prompts to select `React` as the framework and `TypeScript` as the variant. Change directory into the project directory and run
```bash
...
- yup - A form data validation libary.
...
We'll be using [json-server](https://github.com/typicode/json-server) to create our REST API that we'll fetch data from. In the root of the project, create a **db.json** file with the contents
```js
/*db.json*/
...
```
React Query works with zero config and can be customized to meet our application requirements. The above sets up React Query with the default options. We can configure our own config options by passing a config object into the query client as follows
```js
... 
const queryClient = new QueryClient({
...
Then we create a `src/AddTodo.tsx` component with the following contents
```typescript
import React from 'react';
...
// ...
```
We then need to create a `src/hooks/useAddTodo.ts` hook, the one we refence in our `AddTodo` component. Create a `src/hooks/useAddTodo.ts` with the following contents
...
```
## Edit a Todo
We first create a `EditTodo.tsx` component with the contents
```typescript
...
## Conclusion
React Query is a powerful library that makes data fetching and state manamgement in React applications easy and efficient. It provides a simple and intuitive API for managing server state, and has features such as caching, refetching, polling, and more. 

It this article we saw how to create a new React application, and setup React Query, and fetch data from a REST API using Axios. There is a lot of React Query that we haven't touched in this article, such as configuration, testing, etc, as it is outside the scope of this article.